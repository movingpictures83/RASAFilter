...
class HitsDataContainer(list):
    ...
    def tocsv(self, file_path, SQLITE_DATABASE_DIR):
        ...
        with open(file_path, "w") as output_handle:
            ...
            for hit in self:
                ...
                output_handle.write(
                    hit.query_accession+"\t"+hit.subject_accession+"\t"+str(hit.query_start)+"\t"+str(hit.query_end)+"\t"+str(hit.subject_start)+"\t"+str(hit.subject_end)+"\t"+hit.aln_query_seq+"\t"+str(hit.aln_subject_seq)+"\t"+hit.evalue+"\t"+hit.qcovs+"\t"+hit.pident+"\t"+str(hit.staxid)+"\t"+str(hit.match_ranges)+"\t"+str(hit.cigar)+"\t"+str(hit.match_lengths)+"\t"+str(hit.submatch_seqs).replace('\"','')+"\t"+str(hit.acc_seq)+"\t"+hit.pdb_seqsolv+"\t"+hit.pdb_seqnums+"\t"+row[0]+"\t"+row[1]+"\t"+row[2]+"\t"+row[3]+"\t"+row[4]+"\t"+row[5]+"\n"
                    )
                #output_handle.write(
    ...
    def fromcsv(self, file_path):
        ...
        for line in csvfile:
            ...
            pos = 0
            contents[13] = contents[13].strip('][').replace(' ','').split(',')
            contents[12] = []
